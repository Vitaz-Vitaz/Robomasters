class Motor {
    ...
    void set_power(double power) {  // -100 - 100
...
class Smoother {
    ...
    void init() {
        for (uint16_t i = 0; i < sync_smoothing; i++) data[i] = 0;
    }

    double add(double new_data) {
        cur++;
        cur %= sync_smoothing;
        sum -= data[cur];
        sum += new_data;
        data[cur] = new_data;
        return sum / sync_smoothing;
    }
    ...
    private:
    double data[sync_smoothing];
    double sum = 0;
    uint16_t cur = 0;  
};


double sync (Smoother* sync_error, double power1, double power2, int32_t steps1, double last1, int32_t steps2, double last2, double* stop_i, double* stop_le, double* sync_i, double* sync_le) {
    if (power1 == 0) {
        ...
        *stop_le = error;
        ...
        double error = sync_error->add(enc2 - enc1);
        double p = error * sync_cof_p;
        *sync_i += error * sync_cof_i;
        double d = (error - *sync_le) * sync_cof_d;
        *sync_le = error;
        double u = p + *sync_i + d;
        return u;
    }
